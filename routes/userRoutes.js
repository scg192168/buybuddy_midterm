...
router.get("/urls/buyer", (req, res) => {
  ...
  if (!idFromCookie) {
    ...
    return res
      .status(403)
      .send("😒😒😒😒You are not Logged in!!! Log in to use the TinyApp....");
  }
...
router.post("/login/buyer", (req, res) => {
  ...
  userQueries.getUserWithEmail(emailInput).then((userFound) => {
    ...
    if (!userFound) {
      return res
        .status(403)
        .send(
          "😒😒😒😒User account does not exist. Please register a new user account"
        );
    }
...
router.post("/login/seller", (req, res) => {
  ...
  userQueries.getUserWithEmail(emailInput).then((userFound) => {
    ...
    if (userFound.role !== "seller") {
      return res.send({
        error:
          "Either the user you are trying to login with is not a valid seller or user does not exist",
      });
    }
